...
def assignment(f,f2,f3):
    ...
    # making string small
    master['True Class'] = master['True Class'].str.lower()
    student['Label'] = student['Label'].str.lower()

    for i in df.index:
        result = master['True Class'][i] == student['Label'][i]
        if result == False:
            corobr = master['Actual review'][i]
    ...
    master['Actual review'] = act
    master['conflict'] = actresult
    master['Case idd']=master['Case ID']
    # print(master['Case id'])

    finaldflist = [master['Case ID'].apply(str), master['True Class'], student['Label'], master['conflict'], master['Actual review']]

    new_df = pd.concat(finaldflist, axis=1)
    #new_df.to_excel('assign.xlsx', encoding="utf-8")

    true = new_df['conflict'].value_counts()[1]
    false = new_df['conflict'].value_counts()[0]
    total = true + false
    quotient = true / total
